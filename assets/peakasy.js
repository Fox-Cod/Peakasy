...
;(function () {
	...
	var greet =
		h >= 6 && h < 11
			? '🌅 Good morning — your ritual awaits.'
			: h >= 11 && h < 17
				? '⚡ Peak hours ahead — fuel your focus.'
				: h >= 17 && h < 21
					? '🌙 Evening ritual — calm energy.'
...
var RITUALS = {
	focus_morning_black: {
		...
		recipe: [
			'Add 1 heaped tsp to your mug',
			'Pour 180ml water at 170°F',
			'Stir 15 seconds until dissolved',
			'Sip slowly — let clarity arrive',
		],
		bens: [
			'🧠 Peak NGF stimulation',
			'⚡ Sharp clean wakefulness',
			'🎯 Laser focus for deep work',
		],
	},
	focus_morning_oat: {
		...
		recipe: [
			'Brew 1 tsp in 100ml hot water',
			'Steam 80ml oat milk separately',
	...
	focus_morning_almond: {
		...
		recipe: [
			'Dissolve 1 tsp in 120ml hot water',
			'Add 80ml warmed almond milk',
			'Stir well and enjoy warm',
			'Best without sweetener for focus',
		],
		bens: [
	...
	energy_midday_black: {
		...
		recipe: [
			'Use 1.5 tsp for extra strength',
			'Dissolve in only 100ml hot water',
			'Drink in 2-3 sips for fast absorption',
			'Best between 11am and 2pm',
		],
		bens: [
			'⚡ Rapid energy uplift',
			'🏃 Physical stamina boost',
	...
	energy_midday_oat: {
		...
		recipe: [
			'Dissolve 1 tsp in 80ml hot water',
			'Add 100ml cold oat milk over ice',
			'Optional: one medjool date',
			'Best served as an iced latte',
		],
		bens: [
			'⚡ 4-6hr energy sustain',
	...
	energy_midday_almond: {
		...
		recipe: [
			'1 tsp in 120ml water',
			'Top with 60ml almond milk',
			'Enjoy warm or over ice',
			'Avoid after 3pm for sleep quality',
		],
		bens: [
			'⚡ Clean light energy',
	...
	calm_evening_black: {
		...
		recipe: [
			'Use only 0.5 tsp for evening use',
			'Brew in 200ml water at 160°F',
			'Add a drop of vanilla',
			'Hold the mug and breathe',
		],
		bens: [
	...
	calm_evening_oat: {
		...
		recipe: [
			'Half tsp dissolved in 80ml warm water',
			'Heat 120ml oat milk until steaming',
			'Combine, add cardamom',
			'Drink 1hr before wind-down',
		],
		bens: [
			'🌙 Gentle nervous system calm',
	...
	calm_evening_almond: {
		...
		recipe: [
			'0.5 tsp dissolved in 100ml warm water',
			'Top with warm almond milk',
			'Add honey if desired',
			'Best enjoyed in silence',
		],
		bens: [
			'🌿 Adaptogens for balance',
			'🥛 Light and non-bloating',
			'✨ Perfect end-of-day ritual',
		],
	},
}
...
function rPick(el) {
	...
	document
		.querySelectorAll('.ropt[data-s="' + s + '"]')
		.forEach(function (o) {
			o.classList.remove('pick')
		})
...
function rBuild() {
	...
	document.querySelectorAll('.rpan').forEach(function (p) {
	...
	document.getElementById('rr-steps').innerHTML = ritual.recipe
		.map(function (s, i) {
			return (
				'<div class="rr-step"><span class="rr-sn">' +
				(i + 1) +
				'.</span><span>' +
				s +
				'</span></div>'
		...
		.join('')
	document.getElementById('rr-bens').innerHTML = ritual.bens
		.map(function (b) {
			return (
				'<div class="rr-ben"><span>' +
				b.slice(0, 2) +
				'</span>' +
				b.slice(2) +
				'</div>'
...
function rReset() {
	...
	document.querySelectorAll('[id^="rn"]').forEach(function (b) {
		b.disabled = true
	})
...
;(function () {
	...
	document.querySelectorAll('.pic').forEach(function (c) {
		obs.observe(c)
	})
})()
...
;(function () {
	...
	function show() {
		...
		card.innerHTML =
			'<div class="tk-av">' +
			av +
			'</div><div><div class="tk-name">' +
			n +
			' from ' +
			c +
			'</div><div class="tk-act">' +
			a.t +
			' ' +
			a.d +
			' ' +
		...
		setTimeout(function () {
			...
			setTimeout(function () {
				if (ticker.contains(card)) ticker.removeChild(card)
			}, 400)
		}, 4500)
	}
...
;(function () {
	...
	cards.forEach(function (card) {
		...
		card.addEventListener('mousemove', function (e) {
			...
			if (bg)
				bg.style.transform =
					'translate(' + cx * -16 + 'px,' + cy * -16 + 'px)'
			if (mid)
				mid.style.transform =
					'translateZ(28px) translate(' +
					cx * 11 +
					'px,' +
					cy * 11 +
					'px)'
			if (fg)
				fg.style.transform =
					'translateZ(52px) translate(' +
					cx * 22 +
					'px,' +
					cy * 22 +
					'px)'
			if (sh) {
				sh.style.setProperty('--mx', x * 100 + '%')
				sh.style.setProperty('--my', y * 100 + '%')
			}
			if (gcur) {
				gcur.style.left = e.clientX + 'px'
				gcur.style.top = e.clientY + 'px'
			}
		})
	})
})()
...
;(function () {
	...
	var REVS = [
		{
			...
			av: '👩',
			st: '★★★★★',
			txt: '3 months in and my focus is genuinely sharper. The 3pm crash is completely gone.',
			tag: 'Focus',
			ago: '2d ago',
			feat: true,
			photo: 0,
		},
		{
			...
			av: '👨',
			st: '★★★★★',
			txt: 'Tastes like actual premium coffee. My wife started stealing mine so I order double.',
			tag: 'Taste',
			ago: '3d ago',
			feat: false,
			photo: 1,
		},
		{
			...
			av: '🧑',
			st: '★★★★★',
			txt: 'The calm creative clarity is real. As a designer my output has notably improved.',
			tag: 'Clarity',
			ago: '5d ago',
			feat: true,
			photo: 2,
		},
		{
			...
			av: '👨',
			st: '★★★★★',
			txt: 'Replaced the 3pm espresso. No crash, way more balanced and alert all afternoon.',
			tag: 'Energy',
			ago: '1w ago',
			feat: false,
			photo: 3,
		},
		{
			...
			av: '👩',
			st: '★★★★★',
			txt: 'Bought as a gift. Now we both order it every month. Beautiful packaging.',
			tag: 'Gift',
			ago: '1w ago',
			feat: false,
			photo: 4,
		},
		{
			...
			av: '🧑',
			st: '★★★★★',
			txt: 'Tried 6 mushroom coffees. This is the only one where I felt a genuine difference.',
			tag: 'Quality',
			ago: '2w ago',
			feat: true,
			photo: 5,
		},
		{
			...
			av: '👩',
			st: '★★★★★',
			txt: 'Alert without anxiety. Ships fast internationally. Absolutely worth it.',
			tag: 'Focus',
			ago: '2w ago',
			feat: false,
			photo: 6,
		},
		{
			...
			av: '👨',
			st: '★★★★★',
			txt: '6 weeks in, my productivity has genuinely improved. This is not placebo.',
			tag: 'Ritual',
			ago: '3w ago',
			feat: false,
			photo: 7,
		},
		{
			...
			av: '👩',
			st: '★★★★★',
			txt: 'Better skin, better sleep, calmer all day. Replaced my daily latte completely.',
			tag: 'Wellness',
			ago: '3w ago',
			feat: true,
			photo: 0,
		},
		{
			...
			av: '🧑',
			st: '★★★★★',
			txt: 'Stocked up on the 3-tin deal. My mornings have completely transformed.',
			tag: 'Value',
			ago: '1mo ago',
			feat: false,
			photo: 1,
		},
		{
			...
			av: '👩',
			st: '★★★★★',
			txt: 'No bitterness, no aftertaste. Rich coffee with a subtle earthy depth I love.',
			tag: 'Taste',
			ago: '1mo ago',
			feat: false,
			photo: 2,
		},
		{
			...
			av: '👨',
			st: '★★★★★',
			txt: 'Lion Mane is the real deal. Noticed cognitive shift within the first two weeks.',
			tag: 'Science',
			ago: '1mo ago',
			feat: true,
			photo: 3,
		},
		{
			...
			av: '👩',
			st: '★★★★★',
			txt: 'Ordered for my startup team. The entire office has switched from regular coffee.',
			tag: 'Team',
			ago: '5d ago',
			feat: false,
			photo: 4,
		},
		{
			...
			av: '🧑',
			st: '★★★★★',
			txt: 'The ritual aspect is what sold me. This is genuinely premium in every way.',
			tag: 'Ritual',
			ago: '4d ago',
			feat: false,
			photo: 5,
		},
		{
			...
			av: '👩',
			st: '★★★★★',
			txt: 'Smooth taste and I feel the difference daily. Already on my third order.',
			tag: 'Focus',
			ago: '6d ago',
			feat: false,
			photo: 6,
	...
	function makeCard(r, delay) {
		...
		d.innerHTML =
			...
			r.av +
			'</div>' +
			'<div><div class="wrc-nm">' +
			r.nm +
			'</div><div class="wrc-loc">' +
			r.loc +
			'</div></div>' +
			'<div class="wrc-st">' +
			r.st +
			...
			r.txt +
			'"</div>' +
			'<div class="wrc-ft"><span class="wrc-tag">' +
			r.tag +
			'</span><span class="wrc-ago">' +
			r.ago +
			'</span></div>' +
			'</div>'
		return d
		...
		setTimeout(function () {
			...
			if (all.length > 4) {
				...
				old.style.transition =
					'opacity .38s ease, transform .38s ease, max-height .45s ease'
				old.style.opacity = '0'
				...
				setTimeout(function () {
					if (col.contains(old)) col.removeChild(old)
				}, 460)
			}
		}, 600)
	}
...
function galLbNav(dir) {
	...
	if (img) {
		...
		img.style.transition = 'all .18s'
	}
	setTimeout(function () {
		if (img && galImgs[galLbIdx]) {
			img.src = galImgs[galLbIdx].src
			img.style.opacity = '1'
...
function galLbDots() {
	...
	galImgs.forEach(function (_, i) {
		...
		d.style.cssText =
			...
			(i === galLbIdx ? 'var(--gold)' : 'rgba(255,255,255,.3)') +
